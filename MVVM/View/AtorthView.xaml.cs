...
{
    public partial class AtorthView : UserControl
    {
        ...
        public AtorthView() //powoduje wyswietlenia okna w launcherze
        {
            InitializeComponent();
        ...
        private void uninstallButton(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Odinstalowanie gry będzie skutkowało usunięciem wszystkich plików związanych z Atorth: Tales Of Ulkimond. Czy aby na pewno kontynuować? (Upewnij się, że wykonałeś kopię swoich zapisów gry, jeśli natomiast kliknąłeś ten przycisk z powodu niepoprawnego ściągniecia się plików, możesz ten komunikat zignorować.)", "UWAGA!", MessageBoxButton.YesNo, MessageBoxImage.Warning) == MessageBoxResult.No)
                {
                    //jeśli nie to:
                }
                else
                {
                    ...
                    File.Delete(gameZip);
                    MessageBox.Show($"Pomyślnie usunieto: Atorth: Tales Of Ulkimond", "SGSClient", MessageBoxButton.OK, MessageBoxImage.Information);
